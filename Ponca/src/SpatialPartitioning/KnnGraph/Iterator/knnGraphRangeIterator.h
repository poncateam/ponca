 ...
 * : The copy of this iterator would still point to the same KnnGraph reference. So, if the increment operator is used
 * on the iterator, the duplicate will also have its state updated. If we then call the increment operator on the
 * duplicate, the result will be an incorrect value.
 *
