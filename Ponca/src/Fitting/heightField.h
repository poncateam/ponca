...
protected:
    ...
    /// \brief get access to v from local coordinate vector
    PONCA_MULTIARCH [[nodiscard]] inline Scalar& getVFromLocalCoordinates(VectorType& _lq) { return *(_lq.data() + 2); }
};
...
public:
    ...
    /// \brief Second order partial derivative \f$ \frac{\delta }{\delta v} (\frac{\delta h }{\delta u}) (u,v) = h_{uv}
    /// \f$
