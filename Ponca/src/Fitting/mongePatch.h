...
/*!
    ...
    */
    ...
    public:
        ...
        PONCA_MULTIARCH inline bool operator==(const QuadraticHeightField<DataPoint, WFunctor, T>& other) const{
        ...
        PONCA_MULTIARCH inline Scalar height(Scalar u, Scalar v) const {
            return h_uu()*u*u + h_vv()*v*v + h_uv()*u*v + h_u()*u + h_v()*v + h_c();
        }

        PONCA_MULTIARCH inline const Scalar & h_uu () const { return *(m_coeffs.data()); }
        PONCA_MULTIARCH inline const Scalar & h_vv () const { return *(m_coeffs.data()+1); }
        PONCA_MULTIARCH inline const Scalar & h_uv () const { return *(m_coeffs.data()+2); }
        PONCA_MULTIARCH inline const Scalar & h_u  () const { return *(m_coeffs.data()+3); }
        PONCA_MULTIARCH inline const Scalar & h_v  () const { return *(m_coeffs.data()+4); }
        PONCA_MULTIARCH inline const Scalar & h_c  () const { return *(m_coeffs.data()+5); }
...
using MongePatchQuadraticFit =
        FundamentalFormCurvatureEstimator<DataPoint, _WFunctor,
            MongePatchQuadraticFitImpl<DataPoint, _WFunctor,
                MongePatchPrimitive<DataPoint, _WFunctor,
                    QuadraticHeightField<DataPoint, _WFunctor,
                        CovariancePlaneFit<DataPoint, _WFunctor,T>>>>>;
