...
void NormalDerivativeWeingartenEstimator<DataPoint, _NFilter, DiffType, T>::weingartenMap(Matrix2Derived& W) const
{
    ...
    // eigenvalue decomposition of W. However, if N(x) is only an approximation of the normal field of a surface, then
    // N(x) is not necessarily curl-free, and in this case W is not symmetric. In this case, we first have to find an
    // equivalent symmetric matrix W' such that:
    //   K_n(u) = u^T W' u,
    // for any u \in R^2.
