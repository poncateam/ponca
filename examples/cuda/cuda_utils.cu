...
void deepCopyBuffersToDevice( const STLBuffers& hostBuffers, IntermFunctor midProcess, // Input
    StaticBuffers& hostBuffersHoldingDevicePointers, StaticBuffers* const deviceBuffers // Outputs
) {
    ...
    CUDA_CHECK(cudaMemcpy(deviceBuffers, &hostBuffersHoldingDevicePointers,
        sizeof(StaticBuffers), cudaMemcpyHostToDevice)
    );
}
...
void deepCopyKdTreeBuffersToDevice( const KdTreeBuffers& hostBuffers, // Input
    StaticKdTreeBuffers& hostBuffersHoldingDevicePointers, StaticKdTreeBuffers* const deviceBuffers // Outputs
) {
...
void deepCopyKnnGraphBuffersToDevice( const KnnGraphBuffers& hostBuffers, // Input
    StaticKnnGraphBuffers& hostBuffersHoldingDevicePointers, StaticKnnGraphBuffers* const deviceBuffers // Outputs
) {
    deepCopyBuffersToDevice<Traits>( hostBuffers, [&]() {
...
__global__ void fitPotentialAndGradientKernel(
    typename SpatialPartitioning::Buffers* const buffers,
