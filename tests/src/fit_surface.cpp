...
/*!
 \file test/Grenaille/fit_surface.cpp
 \brief Test validity of surface fitting procedure(s)
 */
...
#include <Ponca/src/Fitting/leastSquareSurfaceFit.h>
...
#pragma omp parallel for
    for(int i = 0; i < int(vectorPoints.size()); ++i)
    {
        ...
        VERIFY( fit.isStable() );
        VectorType p1 = vectorPoints[i].pos();
        VectorType p2 = fit.project(vectorPoints[i].pos());
          if(_bAddPositionNoise)
              VERIFY(abs(p1[0] - p2[0]) <= 1.5 && abs(p1[1] - p2[1]) <= 1.5  && abs(p1[2] - p2[2]) <= 1.5 );
          else
             VERIFY(abs(p1[0] - p2[0]) <= .75 && abs(p1[1] - p2[1]) <= .75  && abs(p1[2] - p2[2]) <= .75);
    }
}
...
void callSubTests()
{
    ...
    typedef Basket<Point, WeightSmoothFunc, LeastSquareSurfaceFit> LeastSquareFitSmooth;
    typedef Basket<Point, WeightConstantFunc, LeastSquareSurfaceFit> LeastSquareFitConstant;


    cout << "Testing with perfect surface..." << endl;
    for(int i = 0; i < g_repeat; ++i)
    {
        //Test with perfect surface
...
int main(int argc, char** argv)
{
    ...
    cout << "Test surface fitting for different baskets..." << endl;
